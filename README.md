...
본 저장소는 대회 제출용 코드에서 대용량 모델 가중치와 학습·평가 데이터를 제외하고, 학습 코드, 추론 코드, 모델 구조, 데이터셋 로더, 설정 파일, 실행 환경을 GitHub 포트폴리오용으로 정리한 버전입니다.
...
- CLS 단일 요약의 한계를 보완하는 register-aware feature aggregation 적용
